    ...
    if (dt_user_table.length) {
        var dt_user = dt_user_table.DataTable({
            ...
            ajax: {
                ...
                data: function (d) {
                    return {}; // Send an empty object to remove all default params
                },
            },
            processing: true, // Correct placement
            ...
            columns: [
                ...
                { data: "role" },
                { data: "phone" }, // Phone Number (Nullable)
                { data: "status" }, // Status (active/inactive)
                { data: "is_banned" }, // Ban Status
                { data: null, defaultContent: "" }, // Actions (Custom Column)
            ],
            columnDefs: [
                {
                    // For Responsive
                ...
                },
                {
                    ...
                    targets: 3,
                    render: function (data, type, full, meta) {
                        var $plan = full["current_plan"] ?? 'Free';
                ...
                {
                    // User Status
                    targets: 4, // Adjusted to match the correct column index
                    render: function (data, type, full, meta) {
                        var $status = full["status"];
                        var statusClass = $status === "active" ? "bg-label-success" : "bg-label-danger";
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        var banClass =
                            full["is_banned"] === "true"
                                ? "bg-label-danger"
                                : "bg-label-success";
                        return `<span class="badge ${banClass}">${$banned}</span>`;
                    },
                    // targets: 5,
                    // render: function (data, type, full, meta) {
                    //     var $status = full["status"];
